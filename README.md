...
| 模块 | 说明 |
|------|------|
| 🗺 基础岛课程 | 6 课从零到作品：顺序 → 循环 → 事件 → 条件 → **调试** → 综合发布会，通关解锁；每课另有 ⭐选做挑战 |
| 🧩 积木工作台 | 22 块中文积木（事件/运动/外观/声音/控制/侦测/运算），拖拽即编程，死循环不会卡死 |
| 🎭 小剧场 | 画布舞台：角色、说话气泡、目标旗子、边缘反弹、音效（WebAudio 合成，零素材） |
| 🤖 AI 创意伙伴 | 四模式：💡灵感头脑风暴 / 🆘苏格拉底式三级提示 / 📖生活类比讲解 / 🌟作品点评；支持 🎤 语音说话 |
| ✨ 自由创造岛 | 全积木开放，随时保存作品 |
| 🖼 作品墙 | 作品缩略墙 + 放映模式（含键盘事件，游戏类作品可直接玩）+ ❤️ 家庭点赞 |
| 🏆 结业证书 | 全部通关后解锁，可打印/PDF 贴墙 |
| 🔥 创作之火 | 连续学习天数（温和版激励，不搞强制打卡） |
| 🛡 家长中心 | PIN 进入：学习进度、**全部 AI 对话记录**、伙伴人设、提示严格度、每日时长提醒 |

**为孩子设计的创作细节**：↩️撤销/↪️重做/🧹整理积木按钮、🐢慢速运行（看清程序一步步走）、💾 画布自动存草稿（关掉再回来还在）、👀 魔法代码预览（看看积木变成了什么代码，为以后学 Python 埋种子）、任务完成伙伴自动喝彩（夸努力不夸聪明）。
